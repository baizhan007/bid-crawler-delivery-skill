...
默认只需要提供 `normalize()` 和 `to_row()`。验收表结构用于约束字段含义和唯一性。
...
每个网站按确认采集的 category 拆分 CSV，文件名格式为：
...
只有实际有数据的 category 才生成 CSV；确认采集但本次 0 条的栏目不生成空 CSV。验收报告仍然按整个网站统计总数、重复、字段完整率和栏目覆盖情况。
